...
export default function CourseTypePage() {
  ...
  const { createCourseType, getCourseType } = useApiService();
  ...
  const getUsercourseType = useCallback(async () => {
    const response = await getCourseType();
    setCourseTypeData(response);
    setLoad(false);
  }, [getCourseType]);
  ...
  const handleClose = () => {
    setOpen(false);
    setEdit(false);
    setcourseType({ id: 0, code: '', description: '', name: '', active: false });
    setError('');
  };

  const saveCourseType = async () => {
    const response = await createCourseType(courseType);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = courseTypeData.map((n) => n.name);
        ...
        >
          ...
          >
            <DialogTitle color="#f79520">
              {edit ? 'Update Course Type' : 'Create Course Type'}
            </DialogTitle>
            <DialogContent>
              <Stack spacing={2}>
                <DialogContentText>Create course type to arrange course</DialogContentText>
                <TextField
                  autoFocus
                  margin="dense"
                  id="name"
                ...
                {edit ? (
                  <FormControlLabel
                    control={
                      <Checkbox
                        defaultChecked={courseType.active}
          ...
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              <UserTableHead
                order={order}
                orderBy={orderBy}
                rowCount={courseTypeData.length}
                numSelected={selected.length}
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={77}
                  emptyRows={emptyRows(page, rowsPerPage, courseTypeData.length)}
                />
